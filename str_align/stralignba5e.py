    ...
    for i in range(1, len(str1)): 
        for j in range (1, len(str2)): 
            ...
            if (str1[i],str2[j]) in table: 
                ...
                best_score = score_val #will get the best score as program runs
                if score_val == diag:
                    back[i][j] = "diagonal"
                elif score_val == up:
                    back[i][j] = "up"
...
SIGMA = 5
str1, str2 = readInput()
blo = readBloFile("BLOSUM62.txt")
table = make_lookup(blo)
score,back = process_tables(str1, str2, table)
alig1,alig2 = alignment(str1, str2, back)
